class TMDBklijent {
   ...
   async dohvatiFilm(id : number) {
      let resurs = "/movie/" + id;
      let odgovor = await this.obaviZahtjev(resurs);
      return odgovor;
   }

   async pretraziFilmove(rijeci : string, stranica : number) {
   ...
   async obaviZahtjev(resurs : string, parametri : any = "") {
   ...
   async dajKljucneRijeci(rijeci : string) {
